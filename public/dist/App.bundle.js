...
function ajaxHeart(e) {
	...
	_axios2.default.post(this.action).then(function (res) {
		// this is a form, heart is the name of a button in the form
...
function loadPlaces(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 43.6362579;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -79.49850909999999;
	...
	_axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		...
		var markers = places.map(function (place) {
			var _place$location$coord = _slicedToArray(place.location.coordinates, 2),
			    ...
			    placeLat = _place$location$coord[1]; // creates two variables using array deconstruction
...
function makeMap(mapDiv) {
	...
	// make our map
	var map = new google.maps.Map(mapDiv, defaultMapOptions);
	loadPlaces(map);
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		if (!this.value) {
			// if there is no value provided, quit this function
	...
	searchInput.on('keyup', function (e) {
		// if the user is not pressing up, down or enter, ignore
